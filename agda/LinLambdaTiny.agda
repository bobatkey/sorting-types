...
module Usage where
  ...
  ... | z rewrite use with trans (1#-top z) (1≤th-index-/=-varQCtx np)
  ... | ()
...
module WithBCI {a l} (S : Setoid a l) (Alg : BCI S) where
  ...
  toBCIs {G = G} {var x} (var sub)  = vars x , usageMatch
    ...
    ... | ay rewrite 1≤th-index-/=-varQCtx {rho = 1#} neq with 1≤th-index y G
  ...
  ~~>-preserves-≈ : forall {n d G} {t u : Term n d} {tr : G |-[ tt ] t} {ur : G |-[ tt ] u} f ->
                    t ~~> u -> toBCI tr f ≈ toBCI ur f
  ~~>-preserves-≈ {tr = app split er s0r} {the s1r} f beta = {!!}
  ~~>-preserves-≈ {tr = [ the s0r ]} {ur = s1r} f upsilon = {!!}
